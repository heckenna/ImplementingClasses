...
Authors: David Mutchler, Dave Fisher, Valerie Galluzzi, Amanda Stouder,
         their colleagues and Noah Heckenlively.
...
class Point(object):
    ...
    def move_to(self, x, y):
        self.x = x
        self.y = y
        self.distance += math.sqrt((self.x - self.xtemp) ** 2 + (self.y - self.ytemp) ** 2)
        self.xtemp = x
        self.ytemp = y
        self.count += 1
    ...
    def get_distance_from_start(self):
        return math.sqrt((self.x - self.xi) ** 2 + (self.y - self.yi) ** 2)

    def get_distance_traveled(self):
        return self.distance

    def closer_to(self, pt2, pt3):
        if math.sqrt((self.x - pt2.x)**2 + (self.y - pt2.y)**2) <= math.sqrt((self.x - pt3.x)**2 + (self.y - pt3.y)**2):
            return pt2
        else:
            return pt3

    def halfway_to(self, pt2):
        midx = (pt2.x + self.x)/2
        midy = (pt2.y + self.y)/2
